...
results = zeros(NumTheta, Num1, sizeSpace);

for k=1:1:17
    array = adaptiveBayesian(numParticles, sizeSpace, N, a, resample_threshold, Q, numGuesses, thetaArray(k), s, phases, 0);
    for j=1:1:Num1
        results(k, j, sizeSpace+1) = array(sizeSpace+1, j);
    end
end

save(strcat('resultsTheta', num2str(numVis), '-', num2str(numParticles)), 'results', '-v7.3');
...
%Covariance matrix
function Sigma = estMSE(weights, particleLocations, numParticles, sizeSpace, trueTheta)

visibilities = zeros(4, numParticles);

visibilities(1, :) = 0.900;
visibilities(2, :) = 0.875;
visibilities(3, :) = 0.850;
visibilities(4, :) = 0.765;

locMinusTrue = zeros(sizeSpace, numParticles);

locMinusTrue(1, :) = angularDist(particleLocations(:, 1)', trueTheta);

%Qui ci deve essere un'errore, non può essere (4, :) %FIXME
locMinusTrue(2:sizeSpace, :) = particleLocations(:, 2:sizeSpace)'-visibilities(:, :);

repWeights = repmat(weights', sizeSpace, 1);

Sigma = (locMinusTrue.*repWeights)*locMinusTrue';
...
while((totResources < N) && (iexp < maxExp))
    ...
    estArray(1:sizeSpace, totResources) = estMean(weights, particleLocations, sizeSpace);
    estArray(sizeSpace+1, totResources) = trace(Q*estMSE(weights, particleLocations, numParticles, sizeSpace, trueTheta));
